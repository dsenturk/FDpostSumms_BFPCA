simTables <- function(postEsts,     # list() containing the M posterior estimates of the model components across the
  ...
  ##              of the point estimates and credible intervals/CPEs, respectively. The tables 
  ##              returned from this function correspond to simulation case 1 in Section 
  ##              4 of "Central Posterior Envelopes for Bayesian Functional 
  ...
  ISME_MSE_Table <- function(){
    ...
    kbl(ISME_MSE_values, digits = 4, align = c('l', 'c')) %>%
      ...
      kableExtra::group_rows("ISME", 1, 2 + K * 4, bold = T, underline = TRUE,
                 label_row_css = "text-align: right;", indent = F) %>%
      kableExtra::group_rows("MSE", (2 + K * 4) + 1, ((2 + K * 4) + 1) + K * 2 - 1, 
                             bold = T, underline = TRUE, 
                             label_row_css = "text-align: right;", indent = F)
  }
  ...
  AR <- function(data,             # Posterior estimates (matrix, M x T)
                 credible_int_cpe  # Credible interval estimates or central posterior envelope
                                   # (list, length 2)
    ...
    AR.lower <- ifelse(data.lower < credible_int_cpe$lower, 
                       credible_int_cpe$lower, data.lower)
    AR.upper <- ifelse(data.upper < credible_int_cpe$upper, 
                       data.upper, credible_int_cpe$upper)
  ...
  NAR <- function(data,             # Posterior estimates (matrix, M x T) 
                  credible_int_cpe  # Credible interval estimates or central posterior envelop
                                    # (list, length 2)
                                    # lower: lower bound (vector, T x 1)
                                    # upper: upper bound (vector, T x 1)
                  ){
    ...
    negative_area <- sum(ifelse(  # Area of credible interval that lies outside the estimates
      data.upper < credible_int_cpe$upper, credible_int_cpe$upper - data.upper, 0)) +
      sum(ifelse(
        credible_int_cpe$lower < data.lower, data.lower - credible_int_cpe$lower, 0))
  ...
  AR_NAR_Table <- function(){
    ...
    AR_PP <- c(mean(
      unlist(lapply(1:length(postEsts), function(run){
        AR(postEsts[[run]]$model_components$mu.m, 
           postSumms[[run]]$mu$credible_ints_cpes$pointwise_parametric)
    }))))
    # Eigenfunctions
    for(k in 1:K){
      AR_PP <- c(AR_PP, mean(
        unlist(lapply(1:length(postEsts), function(run){
          AR(postEsts[[run]]$model_components$psi.m[[k]], 
             postSumms[[run]]$psi[[k]]$credible_ints_cpes$pointwise_parametric)
        }))))
    }
    ...
    AR_SP <- c(mean(
      unlist(lapply(1:length(postEsts), function(run){
        AR(postEsts[[run]]$model_components$mu.m, 
           postSumms[[run]]$mu$credible_ints_cpes$simultaneous_parametric)
      }))))
    # Eigenfunctions
    for(k in 1:K){
      AR_SP <- c(AR_SP, mean(
        unlist(lapply(1:length(postEsts), function(run){
          AR(postEsts[[run]]$model_components$psi.m[[k]], 
             postSumms[[run]]$psi[[k]]$credible_ints_cpes$simultaneous_parametric)
        }))))
    }
    ...
    AR_PQ <- c(mean(
      unlist(lapply(1:length(postEsts), function(run){
        AR(postEsts[[run]]$model_components$mu.m, 
           postSumms[[run]]$mu$credible_ints_cpes$pointwise_quantile)
      }))))
    # Eigenfunctions
    for(k in 1:K){
      AR_PQ <- c(AR_PQ, mean(
        unlist(lapply(1:length(postEsts), function(run){
          AR(postEsts[[run]]$model_components$psi.m[[k]], 
             postSumms[[run]]$psi[[k]]$credible_ints_cpes$pointwise_quantile)
        }))))
    }
    ...
    AR_SQ <- c(mean(
      unlist(lapply(1:length(postEsts), function(run){
        AR(postEsts[[run]]$model_components$mu.m, 
           postSumms[[run]]$mu$credible_ints_cpes$simultaneous_quantile)
      }))))
    # Eigenfunctions
    for(k in 1:K){
      AR_SQ <- c(AR_SQ, mean(
        unlist(lapply(1:length(postEsts), function(run){
          AR(postEsts[[run]]$model_components$psi.m[[k]], 
             postSumms[[run]]$psi[[k]]$credible_ints_cpes$simultaneous_quantile)
        }))))
    }
    ...
    AR_MBD <- c(mean(
      unlist(lapply(1:length(postEsts), function(run){
        AR(postEsts[[run]]$model_components$mu.m, 
           postSumms[[run]]$mu$credible_ints_cpes$MBD_cpe)
      }))))
    # Eigenfunctions 
    for(k in 1:K){
      AR_MBD <- c(AR_MBD, mean(
        unlist(lapply(1:length(postEsts), function(run){
          AR(postEsts[[run]]$model_components$psi.m[[k]], 
             postSumms[[run]]$psi[[k]]$credible_ints_cpes$MBD_cpe)
        }))))
    }
    ...
    for(k in 1:K){
      AR_MVD <- c(AR_MVD, sprintf("%.3f", round(mean(
        unlist(lapply(1:length(postEsts), function(run){
          AR(postEsts[[run]]$model_components$psi.m[[k]], 
             postSumms[[run]]$psi[[k]]$credible_ints_cpes$MVD_cpe)
        }))), 3)))
    }
