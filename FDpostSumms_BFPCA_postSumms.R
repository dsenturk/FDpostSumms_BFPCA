postSumms <- function(K,                 # Number of eigencomponents, K (integer) 
                      ...
                      alpha              # Alpha-level of (1 - alpha)100% credible intervals and 
                                         # central posterior envelopes (scalar, 0-1)
                      ){
  #############################################################################
  ...
  ##                credible_ints_cpes: (list, length 5)
  ...
  ##                      lower: lower bound (vector, T x 1)
  ##                      upper: upper bound (vector, T x 1)
  ##                    MBD_cpe: D_{1 - \alpha}{\mu(t)} (list, length 2)
  ##                      lower: lower bound (vector, T x 1)
  ##                      upper: upper bound (vector, T x 1)
  ##              C: (list, length 1)
  ...
  ##                  credible_ints_cpes: (list, length 6)
  ...
  ##                      lower: lower bound (vector, T x 1)
  ##                      upper: upper bound (vector, T x 1)
  ##                    MBD_cpe: D_{1 - \alpha}{\psi_k(t)} (list, length 2)
  ##                      lower: lower bound (vector, T x 1)
  ##                      upper: upper bound (vector, T x 1)
  ##                    MVD_cpe: D^{\star}_{1 - \alpha}{\psi_k(t)} (list, length 2)
  ##                      lower: lower bound (vector, T x 1)
  ##                      upper: upper bound (vector, T x 1)
  ##                ...
  ##                psi_K: (list, length 2)
  ...
  postSumms <- list(
    mu = list(point_ests = list(mu.hat = NA, 
                                mu.MBD = NA), 
              credible_ints_cpes = list(pointwise_parametric = NA, 
  ...
  postSumms$mu$credible_ints_cpes$pointwise_parametric <- pointwise_parametric( 
    model_components$mu.m, alpha = alpha)
  postSumms$mu$credible_ints_cpes$simultaneous_parametric <- simultaneous_parametric(
    model_components$mu.m, alpha = alpha)

  ## ii. Quantile credible intervals
  postSumms$mu$credible_ints_cpes$pointwise_quantile <- confband.pw(
    sample = model_components$mu.m, level = 1 - alpha)
  postSumms$mu$credible_ints_cpes$simultaneous_quantile <- confband.kneib(
    sample = model_components$mu.m, level = 1 - alpha
  )
  ...
  ## i. Parametric credible intervals
  for(k in 1:K){
    postSumms$psi[[k]]$credible_ints_cpes$pointwise_parametric <- pointwise_parametric(
      model_components$psi.m[[k]], alpha = alpha)
    postSumms$psi[[k]]$credible_ints_cpes$simultaneous_parametric <- simultaneous_parametric(
      model_components$psi.m[[k]], alpha = alpha)
  }
 
  ## ii. Quantile credible intervals
  for(k in 1:K){
    postSumms$psi[[k]]$credible_ints_cpes$pointwise_quantile <- confband.pw(
      sample = model_components$psi.m[[k]], level = 1 - alpha)
    postSumms$psi[[k]]$credible_ints_cpes$simultaneous_quantile <- confband.kneib(
      sample = model_components$psi.m[[k]], level = 1 - alpha)
  }
  ...
  depth_cpe <- function(rank,  # Data.frame returned from the MBD() or MVD() functions
  ...
  postSumms$mu$credible_ints_cpes$MBD_cpe <- depth_cpe(
  ...
  for(k in 1:K){
    ...
    postSumms$psi[[k]]$credible_ints_cpes$MBD_cpe <- depth_cpe(
      rank = psi.m.MBD[[k]], model_components$psi.m[[k]], alpha)
    
    ## ii. Calculate MVD credible interval
    postSumms$psi[[k]]$credible_ints_cpes$MVD_cpe <- depth_cpe(
