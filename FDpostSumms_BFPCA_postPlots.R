postPlots <- function(data,              # Sample of posterior estimates, g^{(m)}(t) (matrix, M x T)
                      ...
                      point_est,         # Measure of central tendency used for plot. Takes values 
                                         # c("hat", "tilde", "MBD", "MVD")
                      credible_int_cpe,  # Measure of spread used for plot. Takes values
  ...
  ##              Figures S3, S4 and S5 of "Central Posterior Envelopes for Bayesian 
  ...
  ##              and credible intervals/cpes (shaded area and dashed black lines) they 
  ##              wish to display using the arguments point_est and credible_int_cpe, 
  ##              respectively. To note, the MVD-CPE cannot be calculated
  ##              for the mean function, and as a result, the solid dashed lines 
  ##              are not displayed when k = 0 and credible_int_cpe = "depth". 
  ## Args:        (see above)
  ## Returns:     list()
  ## postPlots Outline:
  ...
  if(credible_int_cpe == "parametric"){
    pointwise.lower <- postSumms$credible_ints_cpes$pointwise_parametric$lower
    pointwise.upper <- postSumms$credible_ints_cpes$pointwise_parametric$upper
    
    simultaneous.lower <- postSumms$credible_ints_cpes$simultaneous_parametric$lower
    simultaneous.upper <- postSumms$credible_ints_cpes$simultaneous_parametric$upper
    
    fill.color <- "#00AFBB"
  } else if (credible_int_cpe == "quantile") {
    pointwise.lower <- postSumms$credible_ints_cpes$pointwise_quantile$lower
    pointwise.upper <- postSumms$credible_ints_cpes$pointwise_quantile$upper
    
    simultaneous.lower <- postSumms$credible_ints_cpes$simultaneous_quantile$lower
    simultaneous.upper <- postSumms$credible_ints_cpes$simultaneous_quantile$upper
    
    fill.color <- "#FC4E07"
  } else if (credible_int_cpe == "depth" && k != 0) {
    pointwise.lower <- postSumms$credible_ints_cpes$MBD_cpe$lower
    pointwise.upper <- postSumms$credible_ints_cpes$MBD_cpe$upper
    
    simultaneous.lower <- postSumms$credible_ints_cpes$MVD_cpe$lower
    simultaneous.upper <- postSumms$credible_ints_cpes$MVD_cpe$upper
  ...
  if(credible_int_cpe == "depth" && k == 0){  # Plot functional depth intervals for mean function (omits MVD interval)
    ggplot() +
